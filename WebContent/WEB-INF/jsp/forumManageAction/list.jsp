		...
		<div id="Title_bar_Head">
			...
			<div id="Title">
				...
				<img border="0" width="13" height="13"
					src="<%=basePath%>style/images/title_arrow.gif" /> 版块管理
			</div>
				...
				<s:iterator value="forumList" status="status">
					<tr class="TableDetail1 template">
						...
						<td><s:a onClick="return delConfirm()"
								action="forumManageAction_delete?id=%{id}">删除</s:a> <s:a
								action="forumManageAction_editUI?id=%{id}">修改</s:a> <s:if
								test="#status.first">
								<span style="color: gray; cursor: pointer">上移</span>
							</s:if> <s:else>
								<s:a action="forumManageAction_moveUp?id=%{id}">上移</s:a>
							</s:else> <s:if test="#status.last">
